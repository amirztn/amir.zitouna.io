# 👋 Bonjour, je suis Amir Zitouna

🎓 Étudiant en **Prépa ATS** et diplômé d’un **BTS CIEL (Cybersécurité, Informatique et Réseaux)**.  
Passionné par **l'informatique, les réseaux et la cybersécurité**, je souhaite intégrer une **école d’ingénieur** afin de concevoir des systèmes innovants et sécurisés.

---

## 🚀 À propos de moi

- 🎓 Étudiant en **Prépa ATS – futur ingénieur**
- 🔐 Passionné par la **cybersécurité et les réseaux**
- 💻 Intéressé par le **développement logiciel et web**
- 🧠 Curieux, rigoureux et toujours prêt à apprendre
- 🥋 **Ceinture noire 1er dan de Taekwondo**

---

## 🛠️ Compétences techniques

### 💻 Langages de programmation
![Python](https://img.shields.io/badge/Python-blue?logo=python)
![C++](https://img.shields.io/badge/C++-00599C?logo=cplusplus)
![PHP](https://img.shields.io/badge/PHP-777BB4?logo=php)
![JavaScript](https://img.shields.io/badge/JavaScript-yellow?logo=javascript)
![HTML](https://img.shields.io/badge/HTML5-orange?logo=html5)
![CSS](https://img.shields.io/badge/CSS3-blue?logo=css3)

---

### 🖥️ Systèmes et réseaux
![Windows](https://img.shields.io/badge/Windows-blue?logo=windows)
![Linux](https://img.shields.io/badge/Linux-black?logo=linux)

- Configuration de **serveurs locaux**
- Gestion de **routeurs et réseaux**
- Installation et maintenance de systèmes

---

### 🔐 Cybersécurité
- SSH
- Gestion des accès
- Sauvegardes sécurisées
- Configuration sécurisée des systèmes

---

### 🗄️ Bases de données
![MySQL](https://img.shields.io/badge/MySQL-blue?logo=mysql)
![MariaDB](https://img.shields.io/badge/MariaDB-brown?logo=mariadb)

---

### ⚙️ Outils
![GitHub](https://img.shields.io/badge/GitHub-black?logo=github)
![VS Code](https://img.shields.io/badge/VSCode-blue?logo=visualstudiocode)
![PyCharm](https://img.shields.io/badge/PyCharm-green?logo=pycharm)

---

# 📂 Projets

## 🌐 Application Web IoT – Visualisation de données de vent

Développement d'une **application web IoT** connectée à une base de données **MariaDB**.

### Technologies utilisées
- Python (Flask)
- MQTT
- MariaDB
- Chart.js
- HTML / CSS / JavaScript

### Fonctionnalités
- 📡 Réception de données IoT
- 📊 Visualisation des données en temps réel
- 🗄️ Stockage des données dans une base MariaDB

---

## 💼 Expérience professionnelle

### Stage Informatique — Arc en Ciel Informatique (Toulouse)  
**8 semaines – 2024**

Missions réalisées :

- 🔧 Réparation et maintenance de matériel informatique
- 🌐 Création et développement d’un site web (HTML / CSS / PHP)
- 🖥️ Installation et configuration de systèmes Windows
- 🌍 Configuration d'environnements réseaux
- 👨‍💻 Support client et assistance technique

---

## 🎓 Formation

**Prépa ATS (Adaptation Technicien Supérieur)**  
📍 Lycée Paul Éluard – Saint-Denis  
2025 – 2026

**BTS CIEL – Cybersécurité, Informatique et Réseaux**  
📍 Lycée Simone de Beauvoir – Toulouse  
2023 – 2025

**Baccalauréat Sciences Techniques – Option S2I**  
📍 Lycée Ibn Charaf – Tunis  
Mention Bien – 2023

---

## 🌍 Langues

- 🇫🇷 Français : courant
- 🇸🇦 Arabe : langue maternelle
- 🇬🇧 Anglais : intermédiaire

---

## 🎯 Centres d'intérêt

- 🥋 Taekwondo – Ceinture noire 1er Dan
- 💻 Informatique et nouvelles technologies
- 🎬 Dessin et montage vidéo

---

## 📫 Me contacter

📧 Email : **amir.zitouna04@gmail.com**

💼 LinkedIn :  
https://www.linkedin.com/in/zitounaamir

---

⭐ N'hésitez pas à explorer mes projets et à me contacter !
